...
unresponsive) — see [docs/ADR-001-adapter-selection.md](chatgpt-desktop-mcp/docs/ADR-001-adapter-selection.md).
...
Runnable examples: [`chatgpt-desktop-mcp/examples/`](chatgpt-desktop-mcp/examples/).
...
locally; summary results live in [CHANGELOG.md](chatgpt-desktop-mcp/CHANGELOG.md).
...
All code lives under [`chatgpt-desktop-mcp/`](chatgpt-desktop-mcp/):
...
docs/ADR-001-adapter-selection.md   # (relative to chatgpt-desktop-mcp/)
```

```text
notes/                       # design doc (zh); test reports are local-only
artifacts/                   # local-only probe evidence (gitignored, not published)
```
...
See [CONTRIBUTING.md](chatgpt-desktop-mcp/CONTRIBUTING.md). Bug reports and capability-test evidence
for new ChatGPT Desktop versions are especially welcome.

## License

[MIT](chatgpt-desktop-mcp/LICENSE) © xukaiming. This project is not affiliated with or endorsed by OpenAI.
